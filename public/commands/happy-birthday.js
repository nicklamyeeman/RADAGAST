...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription("Souhaite un joyeux anniversaire à un membre du discord")
    .addUserOption((option) =>
      option
        .setName("user")
        .setDescription("Membre du discord")
        .setRequired(true)
    ),
  async chatInputExecute(interaction) {
    const member = interaction.guild.members.cache.find(
      (member) => interaction.options.getUser("user").id === member.user.id
    );
    require("axios")
      .request({
      ...
      .then((res) => {
        const embed = new EmbedBuilder()
          ...
          .setImage(
            res.data.data[Math.floor(Math.random() * 25)].images.fixed_height
              .url
          )
          .setTimestamp();
